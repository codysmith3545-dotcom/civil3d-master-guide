---
title: "Grading"
section: "civil3d/grading"
order: 90
visibility: public
tags: [grading, feature-line, grading-group, daylight]
updated: 2026-05-06
---

> **TL;DR**
> 1. **Feature lines** are 3D lines with elevation; they're the backbone of grading.
> 2. **Grading objects** project from a feature line baseline at a slope/distance to a target (surface, elevation, distance) — a **grading group** organizes them and produces a surface.
> 3. For complex sites, prefer **corridors** with a daylight subassembly over freehand grading objects — corridors update predictably; grading objects can be brittle.

## Pages

- [Feature lines](feature-lines.md)
- [Feature line editing (elevation tools, smooth, weed)](feature-line-editing.md)
- [Grading objects (criteria sets, projection, daylight)](grading-objects.md)
- [Grading groups and grading surfaces](grading-groups.md)
- [Site interaction and feature line sites](sites-and-feature-lines.md)
- [Corridor-based grading vs feature-line grading](corridor-vs-feature-line-grading.md)
- [Grading troubleshooting](troubleshooting-grading.md)

## Related

- [Surfaces](../surfaces/index.md)
- [Corridors](../corridors/index.md)
- [Earthwork](../../engineering/earthwork/index.md)
