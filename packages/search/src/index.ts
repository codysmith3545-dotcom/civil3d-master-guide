/**
 * @civil3d-master-guide/search
 *
 * Shared search scoring and excerpt generation for the Civil 3D Master Guide.
 * Both the MCP server and the web app import from here so that search quality
 * improvements apply everywhere at once.
 */
...
/** Minimal page shape required by the search function. */
...
export function buildExcerpt(body: string, queryTokens: string[], maxLen = 240): string {
...
// ---------------------------------------------------------------------------
// Scorer — token overlap with field-weighted boosts
// ---------------------------------------------------------------------------

/**
 * Score and rank pages against a query string.
 *
 * Scoring is based on token overlap with field-specific weights:
 *   - Tag match: 4x  (exact), 2x (partial)
 *   - Title match: 3x (token), 2x (substring)
 *   - Slug/section: 1.5x
 *   - Body: 1x per occurrence (capped at 5)
 *
 * @param pages  Array of pages to search.
 * @param query  Free-text query string.
 * @param opts   Optional settings (limit defaults to 10).
 */
export function searchPages(
  pages: SearchablePage[],
  query: string,
  opts?: SearchOptions | number,
): SearchHit[] {
  ...
  for (const page of pages) {
    ...
    const tags = Array.isArray(page.frontmatter.tags)
      ? page.frontmatter.tags.map((t) => String(t).toLowerCase())
      : [];
